...
import zhaquirks
import zhaquirks.linxura.button
...
async def test_linxura_button(zigpy_device_from_quirk, quirk):
    ...
    linxura_cluster = device.endpoints[1].in_clusters[0xFC00]

    # 0x0002 is also used on manufacturer specific cluster for motion events
    linxura_cluster.update_attribute(0x0002, IasZone.ZoneStatus.Alarm_1)
    ...
    linxura_cluster.update_attribute(0x0002, 0)
