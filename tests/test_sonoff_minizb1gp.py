"""Tests for Sonoff MINI-ZB1GP quirks."""
...
_MINI_ZB1GP_PATH = (
    Path(__file__).resolve().parents[1] / "zhaquirks" / "sonoff" / "mini-zb1gp.py"
)
_MINI_ZB1GP_SPEC = spec_from_file_location(
    "zhaquirks.sonoff.mini_zb1gp_test_module", _MINI_ZB1GP_PATH
)
assert _MINI_ZB1GP_SPEC is not None and _MINI_ZB1GP_SPEC.loader is not None
mini_zb1gp = module_from_spec(_MINI_ZB1GP_SPEC)
sys.modules[_MINI_ZB1GP_SPEC.name] = mini_zb1gp
_MINI_ZB1GP_SPEC.loader.exec_module(mini_zb1gp)

FAST_SCENE_PACKET_MODIFY = mini_zb1gp.FAST_SCENE_PACKET_MODIFY
FAST_SCENE_PACKET_REPORT = mini_zb1gp.FAST_SCENE_PACKET_REPORT
FastSceneProtection = mini_zb1gp.FastSceneProtection
FastSceneState = mini_zb1gp.FastSceneState
SonoffCluster = mini_zb1gp.SonoffCluster
SonoffFastSceneConfigCluster = mini_zb1gp.SonoffFastSceneConfigCluster
decode_fast_scene_payload = mini_zb1gp.decode_fast_scene_payload
encode_fast_scene_payload = mini_zb1gp.encode_fast_scene_payload
fast_scene_array_from_payload = mini_zb1gp.fast_scene_array_from_payload
fast_scene_payload_from_array = mini_zb1gp.fast_scene_payload_from_array
...
async def test_sonoff_minizb1gp_fast_scene_propagation(zigpy_device_from_v2_quirk):
...
async def test_sonoff_minizb1gp_fast_scene_failed_write_does_not_propagate(
...
async def test_sonoff_minizb1gp_fast_scene_write_attributes_logic(
    zigpy_device_from_v2_quirk,
):
...
async def test_sonoff_minizb1gp_apply_custom_configuration_reads_fast_scene(
    zigpy_device_from_v2_quirk,
):
