...
@pytest.mark.parametrize(
    ...
    [
        (None, None, LevelControl.ServerCommandDefs.stop_with_on_off.id, None),  # stop_with_on_off without prior move
        (LevelControl.ServerCommandDefs.move_with_on_off.id, 0, LevelControl.ServerCommandDefs.stop_with_on_off.id, "move_up_release"),  # move up + stop_with_on_off (0x07)
        (LevelControl.ServerCommandDefs.move_with_on_off.id, 0, LevelControl.ServerCommandDefs.stop.id, "move_up_release"),  # move up + stop (0x03)
        (LevelControl.ServerCommandDefs.move.id, 1, LevelControl.ServerCommandDefs.stop.id, "move_down_release"),  # move down + stop (0x03)
        (LevelControl.ServerCommandDefs.move.id, 1, LevelControl.ServerCommandDefs.stop_with_on_off.id, "move_down_release"),  # move down + stop_with_on_off (0x07)
    ],
)
