...
Example: the ZHA-native `AqaraThermostatChildLock` (cluster_id `0xFCC0` = `64704`, attribute `child_lock`) produces unique_id `{ieee}-1-64704-child_lock`. To preserve that under v2, pass `unique_id_suffix="64704-child_lock"` on the corresponding `.switch(...)` call — that yields `{ieee}-1-64704-child_lock` from the v2 path.
