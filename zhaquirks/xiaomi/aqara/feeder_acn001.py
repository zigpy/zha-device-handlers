    ...
    def _update_attribute(self, attrid: int, value: Any) -> None:
        ...
        if attrid == 0x00F7 and isinstance(value, (bytes, types.LVBytes)):
            ...
            try:
                ...
                LOGGER.debug(
                    "[0x%04X] Error updating status_report attribute: %s",
        ...
        if attrid == FEEDER_ATTR and isinstance(value, (bytes, types.LVBytes)):
            ...
            try:
                ...
                LOGGER.debug(
                    "[0x%04X] Error updating feeder_attr: %s", self._get_device_nwk(), e
                )
    ...
    def _encode_schedule(self, schedule_input: Any) -> bytes | None:
        ...
        try:
            ...
            if len(schedule_list) > 5:
                LOGGER.error(
                    "[0x%04X] Too many schedule entries (max 5)", self._get_device_nwk()
                )
                return None
            ...
            for schedule in schedule_list:
                if not isinstance(schedule, dict):
                    LOGGER.error(
                        "[0x%04X] Invalid schedule entry format", self._get_device_nwk()
                    )
                    return None
            ...
            LOGGER.error(
                "[0x%04X] Failed to encode schedule: %s", self._get_device_nwk(), str(e)
            )
            return None
        ...
        for attr, value in attributes.items():
            if attr == ZCL_SCHEDULE or (isinstance(attr, str) and attr == "schedule"):
                try:
                    ...
                    LOGGER.error(
                        "[0x%04X] Error processing schedule: %s",
                        self._get_device_nwk(),
                        str(e),
                    )
                    return [
            ...
            try:
                ...
                LOGGER.warning(
                    "[0x%04X] Unknown attribute: %s", self._get_device_nwk(), attr
                )
                continue
