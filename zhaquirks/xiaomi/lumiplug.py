...
from zigpy.zcl.clusters.general import (
    Basic, PowerConfiguration, DeviceTemperature, Identify, Groups, Scenes,
    OnOff, Time, AnalogInput, BinaryOutput, Ota
)
        ...
        def _update_attribute(self, attrid, value):
            ...
            if attrid == STATUS_TYPE_ATTR:
                ...
                super()._update_attribute(
                    0,
                    '{}'.format(round(float(value), 2))
                )
                if STATUS_TYPE_ATTR in self._currentState and self._currentState[STATUS_TYPE_ATTR] is not None:
                    self.listener_event(
                        'zha_send_event',
                        self,
