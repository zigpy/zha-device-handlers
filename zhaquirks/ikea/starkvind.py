...
class manuSpecificIkeaAirPurifier(CustomCluster):
    ...
    attributes = {
            ...
            0x0006: ("fan_mode", t.uint8_t, True),              # fan mode (Off, Auto, fanspeed 10 - 50)  read/write
            0x0007: ("fan_speed", t.uint8_t, True),             # current fan speed (only fan speed 10-50)
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._update_attribute(0x0003, False) #workaround for empty _attr_cache in https://github.com/zigpy/zigpy/blob/07a3345dd51e54765831cd675fe7eefb57a3aec0/zigpy/zcl/__init__.py#L741 because switch.py - create_entity wont create an entity without
        self._update_attribute(0x0005, False) #workaround for empty _attr_cache in https://github.com/zigpy/zigpy/blob/07a3345dd51e54765831cd675fe7eefb57a3aec0/zigpy/zcl/__init__.py#L741 because switch.py - create_entity wont create an entity without
    ...
    def _update_attribute(self, attrid, value):
        ...
        if attrid == 0x0004:
            if value is not None and value < 5500: # > 5500 = out of scale; if value is 65535 (0xFFFF), device is off
                self.endpoint.device.pm25_bus.listener_event("update_state", value)
        elif attrid == 0x0006:
            if value is not None:
                if value > 9 and value < 51:
                    value = value / 5
                self.endpoint.device.change_fan_mode_ha_bus.listener_event("update_fan_mode_ha", value)
    ...
    async def write_attributes(self, attributes, manufacturer=None):
        ...
        if "fan_mode" in attributes:
            ...
            if fan_mode is not None:
                self.endpoint.device.change_fan_mode_bus.listener_event("update_fan_mode", fan_mode)
...
class PM25Cluster(CustomCluster, PM25):
    ...
    def read_attributes(
        ...
        if "measured_value" in attributes:
            return self.endpoint.device.endpoints[1].in_clusters[64637].read_attributes({"air_quality_25pm"}, allow_cache=allow_cache, only_cache=only_cache, manufacturer=manufacturer)
        else:
