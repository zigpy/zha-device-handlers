...
if True:  # pragma: no cover
    ...
    class QuarterlyAdjustmentState:
        ...
        def __init__(self, values: list[int] | None = None):
            ...
            self.values = list(
                values
                or [QUARTERLY_ADJUSTMENT_DEFAULT_VALUE]
    ...
    def _repeat_to_loop_info(repeat_mode: int, repeat_value: int) -> tuple[int, int]:
        ...
        if repeat_mode == IrrigationPlanRepeat.Interval:
            if not 1 <= repeat_value <= 30:
                raise ValueError(
                    "Irrigation plan interval must be between 1 and 30 days"
                )
    ...
    class CustomSonoffCluster(CustomCluster):
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.on_event(
                ...
                self._handle_quarterly_adjustment_change,
            )
            self.on_event(
        ...
        def _handle_single_irrigation_change(
            ...
            if (
                isinstance(event, AttributeWrittenEvent)
                and event.status != Status.SUCCESS
            ):
                return
            ...
            values = [event.value]
            if (
                isinstance(event, AttributeReadEvent)
                and event.raw_value is not event.value
            ):
    ...
    class SonoffIrrigationPlanConfigCluster(LocalDataCluster):
        ...
        class AttributeDefs(BaseAttributeDefs):
            ...
            plan_fail_safe_duration_min: Final = ZCLAttributeDef(
                id=0x0036, type=t.uint16_t
            )
            plan_total_duration_min: Final = ZCLAttributeDef(id=0x0037, type=t.uint16_t)
            seasonal_adjustment_january: Final = ZCLAttributeDef(
                id=0x0040, type=t.uint8_t
            )
            seasonal_adjustment_february: Final = ZCLAttributeDef(
                id=0x0041, type=t.uint8_t
            )
            seasonal_adjustment_march: Final = ZCLAttributeDef(
                id=0x0042, type=t.uint8_t
            )
            seasonal_adjustment_april: Final = ZCLAttributeDef(
                id=0x0043, type=t.uint8_t
            )
            ...
            seasonal_adjustment_august: Final = ZCLAttributeDef(
                id=0x0047, type=t.uint8_t
            )
            seasonal_adjustment_september: Final = ZCLAttributeDef(
                id=0x0048, type=t.uint8_t
            )
            seasonal_adjustment_october: Final = ZCLAttributeDef(
                id=0x0049, type=t.uint8_t
            )
            seasonal_adjustment_november: Final = ZCLAttributeDef(
                id=0x004A, type=t.uint8_t
            )
            seasonal_adjustment_december: Final = ZCLAttributeDef(
                id=0x004B, type=t.uint8_t
            )
        ...
        def _update_all_attributes(self) -> None:
            ...
            updates = {
                ...
                self.AttributeDefs.weekday_sunday.id: int(
                    bool(self._weekday_mask & 0x01)
                ),
                self.AttributeDefs.weekday_monday.id: int(
                    bool(self._weekday_mask & 0x02)
                ),
                self.AttributeDefs.weekday_tuesday.id: int(
                    bool(self._weekday_mask & 0x04)
                ),
                ...
                self.AttributeDefs.weekday_friday.id: int(
                    bool(self._weekday_mask & 0x20)
                ),
        ...
        def _current_water_flow_unit(self) -> int:
            ...
            if unit is None and hasattr(
                self.endpoint, "sonoff_single_irrigation_config"
            ):
        ...
        def _plan_from_current_config(self) -> IrrigationPlan:
            ...
            start_datetime = _seconds_from_midnight(
        ...
        def _validate_plan_before_send(self) -> None:
            ...
            if int(self._irrigation_mode) != int(
                IrrigationPlanMode.Duration_With_Interval
            ):
                return
    ...
    class SonoffDurationOnlyIrrigationPlanConfigCluster(
    ...
    if True:  # pragma: no cover
        (
            ...
            .number(
                ...
                unit=UnitOfTime.MINUTES,
                mode="box",
                translation_key="duration_only_manual_single_irrigation_total_duration",
