    ...
    def _update_attribute(self, attrid, value):
        ...
        if attrid in self.TEMPERATURE_ATTRS:
            self.endpoint.device.thermostat_bus.listener_event(
                ...
                else self.TEMPERATURE_ATTRS[attrid][1](
                    value
                ),  # decidegree to centidegree
            )
            ...
            self.endpoint.device.battery_bus.listener_event(
                "boost_duration_seconds", value
            )
        elif attrid == ZONNSMART_UPTIME_TIME_ATTR:
            self.endpoint.device.battery_bus.listener_event(
                "uptime_duration_hours", value
            )
...
class ZONNSMARTThermostat(TuyaThermostatCluster):
    ...
    def map_attribute(self, attribute, value):
        ...
        if attribute == "occupied_heating_setpoint":
            # centidegree to decidegree
            return {ZONNSMART_TARGET_TEMP_ATTR: round(value / 10)}
        if attribute in ("programing_oper_mode", "occupancy"):
            if attribute == "occupancy":
                occupancy = value
                oper_mode = self._attr_cache.get(
        ...
        if attribute == "system_mode":
            return {
                ZONNSMART_MODE_ATTR: self._attr_cache.get(
                    self.attridx["operation_preset"], 0
                )
            }
...
class ZonnsmartTV01_ZG_Type1(TuyaThermostat):
